...
%# =================================================================#
	...
	emis=0.99*ones(1,length(zenithangle));
	...
	ix=find(zenithangle <= 140 & zenithangle >= 120);
	if length(ix)>0,
		nadirangle=180-zenithangle(ix);
		% --- INTERPOLATE TO SEA ANGLE ---
		emis(ix) = interp1(a(:,1),a(:,2),nadirangle);
	end
	return
end
